...
import { Sparkles, ShoppingCart, Phone, MapPin, Menu, X, Heart, Star, Check, Plus, Gift, Facebook, Instagram, Twitter, Youtube, Send, ArrowRight } from "lucide-react";
import { toast } from "sonner";
import { fetchAdminCatalog, fetchCustomizedServices, type AdminCustomizedService } from "@/api/admin-api";
import {
  Category,
  Service,
  CartItem,
...
function ServicesComponent() {
    ...
    fetchAdminCatalog(ctrl.signal)
      .then((catalog) => {
        if (catalog.categories?.length) {
          ...
          const defaultCat = searchParams.category && merged.some(c => c.id === searchParams.category)
  ...
  useEffect(() => {
    try { localStorage.setItem("thedeepcleanerz_cart_v1", JSON.stringify(cart)); } catch {}
  }, [cart]);
  ...
  <nav className="hidden items-center gap-8 lg:flex">
                ...
                {categories.map((cat) => (
                  ...
                  >
                    ...
                    <div className={`grid h-5 w-5 place-items-center rounded border transition-colors ${
                      selectedCatId === cat.id ? "bg-emerald-600 border-emerald-600 text-white" : "border-slate-300 bg-white"
                    }`}>
          ...
          <div className="w-full lg:w-9/12 flex-1 space-y-6">
            ...
            {isLoading ? (
              ...
              <div className="flex flex-col items-center justify-center p-12 bg-white border border-slate-100 rounded-3xl text-center">
                ...
                <p className="text-2xs text-slate-400 mt-1">Please add services to this category from your Admin panel.</p>
              </div>
            ) : (
      ...
      {/* FOOTER */}
      <footer className="bg-navy text-cream pt-16 pb-8 border-t border-gold/20 relative overflow-hidden">
